...
impl AesKeySched {
    pub fn aes128_set_encrypt_key(&mut self, key: [u8; 16]) {
        const RCONST: [u32; 11] = [
        ...
        for i in 0..NK {
            self.rd_key[i] = ((key[NB * i] as u32) << 24)
        ...
        for i in NK..(NB * (NR + 1)) {
            let mut temp = self.rd_key[i - 1];
            if i % NK == 0 {
                temp = subword(rotword(temp)) ^ RCONST[i / NK];
            }
            self.rd_key[i] = self.rd_key[i - NK] ^ temp;
        }
    }
}
...
fn main() {
    ...
    key_sched.aes128_set_encrypt_key(key);
